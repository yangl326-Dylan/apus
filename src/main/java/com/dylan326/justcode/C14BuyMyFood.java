 ...
 * 到周末了，小周为了犒劳辛苦了一周得自己，打算去超市买点自己喜欢的零食吃。由于各个超市的备货情况不同，小周可能需要多走几个超市才能买完自己想要的零食。
 * 小周有 n 种心仪的零食，小区附近有 m 家超市，第 i 家超市售卖 ct_i 种零食，分别为第 ai,1 , ai,2 , ... , ai,ct_i  种，小周想要知道至少要需要访问几家超市，才能买齐这 n 种零食？
 * <p>
 * 5 4 （n，m）
 * 1 2  第一家有1，2
 * 3 1 3 5
...
public class C14BuyMyFood {
        ...
        for (int i = startM; i < m; ) {
            for (int j = 0; j < n; j++) {
                ...
                if (checkResult(result)) {
                    counter = (Math.min(counter, (i - startM + 1)));
    ...
    public static int buyMyFoodDP(int[][] input) {
        int m = input.length; // m家超市
        int n = input[0].length; // n种零食
        int[] food = new int[n];
        for (int i = 0; i < food.length; i++) {
            food[i] = i + 1;
        }
        int[][] dp = new int[m][n]; // i,j dp[i][j]= dp[i-1][j] , dp[i][j] = dp[i][j - food[i]]
        return 0;
    }
